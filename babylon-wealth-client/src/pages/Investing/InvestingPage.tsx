...
function buildStackedPassive(records: { receivedDate: string; sourceName: string; amount: number }[]) {
...
function CurrencyInput({
  defaultValue = 0, onChange, className,
}: { defaultValue?: number; onChange: (v: number) => void; className?: string }) {
  ...
  return (
    <input
      ref={inputRef} className={className} type="text" inputMode="decimal"
      value={text} onChange={handleChange} onBlur={handleBlur} placeholder="0.00"
    />
  );
}
...
export function InvestingPage() {
  ...
  // Monthly = sum of the most recent month present in passive income data
  // Yearly  = monthly × 12
  const { monthlyPassive, yearlyPassive } = useMemo(() => {
    if (incomeAll.length === 0) return { monthlyPassive: 0, yearlyPassive: 0 };
    const latestKey = [...new Set(incomeAll.map(r => {
      const d = new Date(r.receivedDate);
      return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2,'0')}`;
    }))].sort().at(-1)!;
    const [y, m] = latestKey.split('-').map(Number);
    const monthly = incomeAll
      .filter(r => {
        const d = new Date(r.receivedDate);
        return d.getFullYear() === y && d.getMonth() + 1 === m;
      })
      .reduce((s, r) => s + r.amount, 0);
    return { monthlyPassive: monthly, yearlyPassive: monthly * 12 };
  }, [incomeAll]);
  ...
  const invDraftRef      = useRef(invDraft);    invDraftRef.current    = invDraft;
  const invEditBankRef   = useRef(invEditBank); invEditBankRef.current = invEditBank;

  function startInvEdit(row: InvestmentResponseDto) {
    setInvEditId(row.id);
    setInvDraft({ customLabel: row.customLabel, investmentType: row.investmentType, currentValue: row.currentValue, bankId: row.bankId, ticker: row.ticker });
    setInvEditBank(row.bankId ? { id: row.bankId, name: row.bankName ?? '', logoUrl: row.bankLogoUrl, type: '' } : null);
  }
  ...
  async function saveInvEdit(row: InvestmentResponseDto) {
    ...
    try {
      ...
      setInvEditId(null); setInvDraft({}); setInvEditBank(null);
    } finally { setInvSaving(false); }
  }

  function cancelInvEdit() { setInvEditId(null); setInvDraft({}); setInvEditBank(null); }
  ...
  async function handleAddInvestment() {
    ...
    try {
      ...
      setAddInvBank(null); setAddInvKey(k => k + 1); setShowAddInv(false);
  ...
  const investmentColumns = useMemo<ColumnDef<InvestmentResponseDto, unknown>[]>(() => [
        ...
        if (invEditId === row.original.id) {
          return (
            <BankSearchInput
              ...
              onChange={(b) => { setInvEditBank(b); setInvDraft(d => ({ ...d, bankId: b?.id ?? null })); }}
            />
          );
        }
    ...
    },
    {
      ...
      cell: ({ row }) => {
        if (invEditId === row.original.id) {
          return (
            <input key={`${row.original.id}-inv-label`} className="lt__edit-input lt__edit-input--wide"
    ...
    },
    {
      ...
      cell: ({ row }) => {
        if (invEditId === row.original.id) {
          return (
            <CurrencyInput key={`${row.original.id}-inv-value`} className="lt__edit-input lt__edit-input--number"
        ...
        if (invEditId === row.original.id) {
          return (
            <div className="lt__action-group">
              <button className="lt__action-btn lt__action-btn--apply" onClick={() => saveInvEdit(row.original)} disabled={invSaving}>
                {invSaving ? '…' : 'Save'}
              </button>
              <button className="lt__action-btn lt__action-btn--cancel" onClick={cancelInvEdit}>Cancel</button>
            </div>
          );
        }
        return (
          <div className="lt__action-group">
            <button className="lt__action-btn lt__action-btn--edit" onClick={() => startInvEdit(row.original)}>Edit</button>
            <button className="lt__action-btn lt__action-btn--delete" onClick={() => handleDeleteInv(row.original)}>✕</button>
          </div>
        );
      },
    },
  ...
  // ── Portfolio Over Time (localStorage) ───────────────────────
  ...
  const addPortRow = () => {
    ...
    const pendingNewCats = [...new Set(portRows.map(r => r.category.trim()).filter(Boolean))].filter(c => !existingCats.includes(c));
    if (existingCats.length + pendingNewCats.length >= 6) { setPortError('Maximum 6 categories reached.'); return; }
  ...
  const handleSavePortEntries = () => {
    ...
    if (existingCats.length + newCats.length > 6) { setPortError('Adding these entries would exceed 6 categories.'); return; }
    const toAdd: PortfolioEntry[] = validRows.map(r => ({ id: genId(), month: portMonth, year: portYear, category: r.category.trim(), amount: parseFloat(r.amount) }));
    const next = [...portfolioEntries, ...toAdd];
    setPortfolioEntries(next); savePortfolioEntries(next);
    setPortRows([newPortRow()]); setShowPortEntry(false);
  };
  ...
  const addPassiveRow = () => {
    ...
    const pendingNewCats = [...new Set(passiveRows.map(r => r.source.trim()).filter(Boolean))].filter(c => !existingCats.includes(c));
    if (existingCats.length + pendingNewCats.length >= 6) { setPassiveError('Maximum 6 categories reached.'); return; }
  ...
  const handleSavePassiveEntries = async () => {
    ...
    if (existingCats.length + newCats.length > 6) { setPassiveError('Adding these entries would exceed 6 sources.'); return; }
    try {
      ...
      setPassiveRows([newPassiveRow()]); setShowPassiveEntry(false);
    } catch {
  ...
  const handleSeeProjection = () => setProjData(projectGrowth(projMonthly, projRate, projYears, totalPortfolio));
      ...
      <div className="acc-section">
        ...
        {showAddInv && (
          <div className="acc-add-form">
            ...
            <label className="acc-add-field">
              <span className="acc-add-field-label">&nbsp;</span>
              <select className="acc-add-select" value={addInvForm.investmentType}
            ...
            <label className="acc-add-field">
              ...
              <CurrencyInput key={addInvKey} className="acc-add-input acc-add-input--number"
                onChange={v => setAddInvForm(f => ({ ...f, currentValue: v }))} />
            </label>
            <label className="acc-add-field">
              <span className="acc-add-field-label">&nbsp;</span>
              <button className="acc-btn acc-btn--primary acc-btn--sm" type="button"
                onClick={handleAddInvestment} disabled={addingInv}>
        ...
        <div className="inv-charts-row">
          ...
          <div className="inv-chart-card">
            <div className="inv-chart-card__header">
              ...
              <button className="inv-btn inv-btn--add inv-btn--sm"
                onClick={() => { setShowPortEntry(v => !v); setPortRows([newPortRow()]); setPortError(null); }}>
            ...
            {showPortEntry && (
              <div className="inv-entry-form">
                  ...
                  {portRows.map((row, i) => (
                    <div key={row.id} className="inv-entry-form__catrow">
                      ...
                      <div className="inv-entry-form__field inv-entry-form__field--grow">
                        ...
                        <input type="text" placeholder="e.g. 401k, Brokerage, Crypto"
                          value={row.category} onChange={e => updatePortRow(row.id, 'category', e.target.value)} />
                      </div>
                      ...
                      <button className="inv-icon-btn inv-icon-btn--danger inv-entry-form__catrow-remove"
                        style={{ visibility: portRows.length > 1 ? 'visible' : 'hidden', marginTop: i === 0 ? 16 : 0 }}
                        onClick={() => removePortRow(row.id)} title="Remove">×</button>
                    </div>
                  ))}
                </div>
                <button className="inv-btn inv-btn--ghost inv-btn--sm inv-entry-form__addrow" onClick={addPortRow}>
                  + Add Category
                </button>
                ...
                <button className="inv-btn inv-btn--primary inv-btn--sm" onClick={handleSavePortEntries}>
                  Save {portRows.filter(r => r.category.trim()).length > 1 ? `${portRows.filter(r => r.category.trim()).length} Entries` : 'Entry'}
                </button>
              </div>
            )}
            ...
            {portfolioChart.data.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={portfolioChart.data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fill: 'var(--color-text-muted)', fontSize: 11 }} axisLine={false} tickLine={false} width={56}
                    tickFormatter={v => v >= 1000 ? `$${(v/1000).toFixed(0)}k` : `$${v}`} />
                  <RechartTooltip content={<StackedTooltip />} cursor={{ fill: 'rgba(255,255,255,0.04)' }} />
                  {portfolioChart.categories.map((cat, i) => (
                    <Bar key={cat} dataKey={cat} stackId="a" fill={CAT_COLORS[i]}
                      radius={i === portfolioChart.categories.length - 1 ? [4,4,0,0] : [0,0,0,0]} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            )}
          ...
          <div className="inv-chart-card">
            <div className="inv-chart-card__header">
              ...
              <button className="inv-btn inv-btn--add inv-btn--sm"
                onClick={() => { setShowPassiveEntry(v => !v); setPassiveRows([newPassiveRow()]); setPassiveError(null); }}>
                  ...
                  {passiveRows.map((row, i) => (
                    <div key={row.id} className="inv-entry-form__catrow inv-entry-form__catrow--passive">
                      ...
                      <div className="inv-entry-form__field inv-entry-form__field--grow">
                        ...
                        <input type="text" placeholder="e.g. SCHD Dividend, Bond Interest"
                          value={row.source} onChange={e => updatePassiveRow(row.id, 'source', e.target.value)} />
                      </div>
                      ...
                      <button className="inv-icon-btn inv-icon-btn--danger inv-entry-form__catrow-remove"
                        ...
                        onClick={() => removePassiveRow(row.id)} title="Remove">×</button>
                    </div>
                  ))}
                </div>
                <button className="inv-btn inv-btn--ghost inv-btn--sm inv-entry-form__addrow" onClick={addPassiveRow}>
                  + Add Source
                </button>
                {passiveError && <div className="inv-form-error">{passiveError}</div>}
                <button className="inv-btn inv-btn--primary inv-btn--sm" onClick={handleSavePassiveEntries}
                  disabled={createIncome.isPending}>
                  {createIncome.isPending
                    ? 'Saving…'
                    : `Save ${passiveRows.filter(r => r.source.trim()).length > 1 ? `${passiveRows.filter(r => r.source.trim()).length} Entries` : 'Entry'}`}
                </button>
              </div>
            )}
            ...
            {passiveLoading ? (
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={passiveChart.data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} />
                  <XAxis dataKey="label" tick={{ fill: 'var(--color-text-muted)', fontSize: 11 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: 'var(--color-text-muted)', fontSize: 11 }} axisLine={false} tickLine={false} width={52}
                    tickFormatter={v => `$${v}`} />
                  <RechartTooltip content={<StackedTooltip />} cursor={{ fill: 'rgba(255,255,255,0.04)' }} />
                  {passiveChart.categories.map((cat, i) => (
                    <Bar key={cat} dataKey={cat} stackId="b" fill={CAT_COLORS[i]}
                      radius={i === passiveChart.categories.length - 1 ? [4,4,0,0] : [0,0,0,0]} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            )}
                ...
                <div className="inv-entry-list__rows">
                  {[...incomeAll].map(e => (
                    <div key={e.id} className="inv-entry-list__row">
                      ...
                      <span className="inv-entry-list__amt">{formatCurrency(e.amount)}</span>
                      <button className="inv-icon-btn inv-icon-btn--danger"
                        onClick={() => handleDeletePassiveEntry(e.id)} disabled={deleteIncome.isPending}>×</button>
                    </div>
                  ))}
                </div>
              </details>
          ...
          <div className="inv-projector__controls">
            ...
            <div className="inv-projector__control">
              ...
              <div className="inv-projector__input-wrap">
                <span>$</span>
                <input type="number" min={0} step={50} value={projMonthly}
            ...
            <div className="inv-projector__control">
              ...
              <div className="inv-projector__input-wrap">
                <input type="number" min={0} max={50} step={0.5} value={projRate}
            ...
            <div className="inv-projector__control">
              ...
              <div className="inv-projector__input-wrap">
                <input type="number" min={1} max={60} step={1} value={projYears}
              ...
              <div className="inv-projector__chart-wrap">
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={projData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="totalValue" name="Total w/ Growth"
                      stroke="var(--color-positive)" strokeWidth={2.5} dot={false}
                      activeDot={{ r: 5, fill: 'var(--color-positive)' }} />
                    <Line type="monotone" dataKey="contributed" name="Contributions Only"
              ...
              {finalProj && (
                <div className="inv-projector__summary">
                  <div className="inv-projector__summary-item">
                    <span className="inv-projector__summary-label">In {projYears} year{projYears !== 1 ? 's' : ''}</span>
                  ...
                  <div className="inv-projector__summary-item">
                    ...
                    <span className="inv-projector__summary-value">{formatCurrency(finalProj.contributed)}</span>
                  </div>
                  <div className="inv-projector__summary-divider" />
                  <div className="inv-projector__summary-item">
